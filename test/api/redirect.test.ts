...
void describe('/redirect', () => {
  ...
  void it('GET rejects target URL when an allow-listed URL is only part of the query string', async () => {
    const res = await request(app)
  ...
  void it('GET rejects external target URL when an allow-listed URL is embedded in the query string', async () => {
    const res = await request(app)
      .get('/redirect?to=https%3A%2F%2Fevil.example%2F%3Fnext%3Dhttps%3A%2F%2Fgithub.com%2Fjuice-shop%2Fjuice-shop')
      .redirects(0)
    assert.equal(res.status, 406)
    assert.ok(res.headers['content-type']?.includes('text/html'))
    assert.ok(res.text.includes('Unrecognized target URL for redirect'))
