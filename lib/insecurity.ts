...
export const generateCoupon = (discount: number, date = new Date()) => {
  const coupon = utils.toMMMYY(date) + '-' + discount.toString().padStart(2, '0')
  return coupon + '-' + hmac(coupon).slice(0, 16)
}
...
export const discountFromCoupon = (coupon?: string) => {
  ...
  if (hasValidFormat(coupon) == null) {
    return undefined
  }
  const [validity, discount, signature] = coupon.split('-')
  const unsignedCoupon = validity + '-' + discount
  if (hmac(unsignedCoupon).slice(0, 16) !== signature) {
    return undefined
  }
...
function hasValidFormat (coupon: string) {
  return coupon.match(/^(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[0-9]{2}-[0-9]{2}-[0-9a-f]{16}$/)
}
