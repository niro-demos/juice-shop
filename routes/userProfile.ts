...
export function getUserProfile () {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    // The username is always rendered as a plain, literal text label. It
    // must never be evaluated as code (no eval() or any other execution
    // sink) — the leading backslash also neutralizes Pug's own `#{...}`
    // interpolation syntax, since `_username_` below is substituted directly
    // into the raw template source before it is compiled.
